...
namespace Leitura
{
    class SolucaoFluxoNewtonRaphsonDesacopladoRapido
    {
        ...
        public Sparse2DMatrix<int, int, double> B1_matriz = new Sparse2DMatrix<int, int, double>();
        ...
        public Sparse2DMatrix<int, int, double> Jacobiano_B1 = new Sparse2DMatrix<int, int, double>();
        public Sparse2DMatrix<int, int, double> Jacobiano_B11 = new Sparse2DMatrix<int, int, double>();

        public SolucaoFluxoNewtonRaphsonDesacopladoRapido(Sparse2DMatrix<int, int, double> G_mat,
                                         Sparse2DMatrix<int, int, double> B_mat,
                                         Sparse2DMatrix<int, int, double> B1_mat,
                                         Barra Bar,
                                         Linha Lin)
        {
            G_matriz = G_mat;
            ...
            while (true)
            {
                ...
                if (KP == 1)
                {
                    ...
                    for (int k = 1; k <= Barra.NBarra.Count; k++)
                    {
                        ...
                        {
                            case 2:
                                Jacobiano_B1[k, k] = 1e40;
                                break;

                            case 0:
                                Jacobiano_B1[k, k] = B1_matriz[k, k];
                                break;
                    ...
                    for (int i = 1; i <= Linha.DaBarra.Count; i++)
                    {
                        ...
                        Jacobiano_B1[k, m] = B1_matriz[k, m];
                        Jacobiano_B1[m, k] = B1_matriz[m, k];
                    }
                    ...
                    for (int j = 0; j < Barra.NBarra.Count; j++)
                    {
                        ...
                        {
                            Jacobiano_B1[j, i] = Jacobiano_B1[j + 1, i + 1];
                        }
                    }

                    LinearEquationSolver.Solve(D_P_Solver.Count, Jacobiano_B1, D_P_Solver, D_theta);
                ...
                if (KQ == 1)
                {
                    ...
                    for (int k = 1; k <= Barra.NBarra.Count; k++)
                    {
                        ...
                        {
                            case 2:
                                Jacobiano_B11[k, k] = 1e40;
                                break;

                            case 0:
                                Jacobiano_B11[k, k] = -B_matriz[k, k];
                                break;
                    ...
                    for (int i = 1; i <= Linha.DaBarra.Count; i++)
                    {
                        ...
                        Jacobiano_B11[k, m] = -B_matriz[k, m];
                        Jacobiano_B11[m, k] = -B_matriz[m, k];
                    }
                    ...
                    for (int j = 0; j < Barra.NBarra.Count; j++)
                    {
                        for (int i = 0; i < Barra.NBarra.Count; i++)
                        {
                            Jacobiano_B11[j, i] = Jacobiano_B11[j + 1, i + 1];
                        }
                    }
                    ...
                    LinearEquationSolver.Solve(D_Q_Solver.Count, Jacobiano_B11, D_Q_Solver, D_V);
